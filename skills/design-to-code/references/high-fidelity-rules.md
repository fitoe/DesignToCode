...
- Maintain an icon anatomy map before coding. For every icon-like mark, record source role, chosen library class, size, color, container/background yes/no, stroke/fill style, and inline alignment. If the source shows a bare icon, certification mark, logo-like trust mark, or official-looking approval badge, do not wrap it in a colored tile, bordered card, badge frame, or decorative container unless the approved source explicitly shows that frame. If an inline dropdown/arrow/refresh is visible, use an icon library class instead of text glyphs like `⌄`, `›`, or `↻` unless the source is explicitly typographic.
